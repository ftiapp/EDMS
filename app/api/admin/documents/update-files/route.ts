...
// POST /api/admin/documents/update-files
// แอดมินอัปเดตไฟล์แนบของเอกสารได้ โดยไม่ตรวจ owner_email
export async function POST(req: Request) {
  try {
        ...
        try {
          await new Promise<any>((resolve, reject) => {
            const previewStream = cloudinary.uploader.upload_stream(
              ...
              (error, result) => {
                if (error) {
                  console.error("Preview upload error (admin update-files):", error);
                  reject(error);
                } else {
                  resolve(result);
                }
          ...
          console.error("Failed to create PDF preview on admin update:", previewError);
        }
      }

      const fileUrl: string = (uploadResult as any).secure_url;
    ...
    const [result] = await db.execute(
      ...
      [JSON.stringify(finalUrls), JSON.stringify(finalNames), id]
    );

    const anyResult: any = result;
    if (!anyResult || !anyResult.affectedRows) {
      return NextResponse.json(
        { message: "ไม่พบเอกสารสำหรับแก้ไขไฟล์แนบ (admin)" },
        { status: 404 }
      );
    }
    ...
    console.error("Admin update files error:", error);
    return NextResponse.json(
      { message: "เกิดข้อผิดพลาดระหว่างการอัปเดตไฟล์แนบ (admin)" },
      { status: 500 }
    );
  }
}
